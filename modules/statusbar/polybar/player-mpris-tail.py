...
def getActivePlayer():
    players = [{'name': player, 'status': getPlayerStatus(player)}
               for player in listPlayers()]
    playing = [p['name'] for p in players if p['status'] == 'Playing']
    paused = [p['name'] for p in players if p['status'] == 'Paused']
...
class PlayerStatus:
    ...
    def _print_song(self):
        self._print_flush('{} {}'.format(self._icon, self._title))
